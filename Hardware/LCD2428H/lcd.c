...
void LCD_GPIO_Init(void)
{
    ...
    HAL_GPIO_WritePin(GPIOB,GPIO_PIN_0,GPIO_PIN_SET);	  //PB0置1 	
	  ...
	  HAL_GPIO_WritePin(GPIOB,GPIO_PIN_12,GPIO_PIN_SET);	//PB12置1 
		...
		SPI2_SetSpeed(SPI_BAUDRATEPRESCALER_4); //设置高速模式
...
////{		 			 
//// 
////  while((SPI2->SR & SPI_I2S_FLAG_TXE) == RESET);  //while (SPI_I2S_GetFlagStatus(SPI2, SPI_I2S_FLAG_TXE) == RESET){}//等待发送区空  
////	
////	SPI2->DR = TxData;                               //SPI_I2S_SendData(SPI2, TxData); //通过外设SPIx发送一个byte  数据
////	
...
void Demo_Menu(void)
{
	...
	Color_Test();       //颜色填充测试
	Font_Test();        //字体字形显示测试--使用STM32片上字库
