/**
 * @file    ui_service.c
 * @brief   LVGL display service — init, overlay, periodic refresh.
 */
...
int ui_service_init(void)
{
    ...
    lv_port_indev_init();
    ui_overlay_init();
    /* lv_port_tick_init() must be called from a task context,
     * not here — it's called at the top of the display task. */
    return 0;
}
